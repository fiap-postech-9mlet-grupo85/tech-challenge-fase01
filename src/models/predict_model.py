...
def load_artifacts():
    ...
    if _preprocessor is None:
        ...
        if not os.path.exists(preprocessor_path):
            ...
            raise FileNotFoundError(
                f"Artefato {preprocessor_path} não encontrado. Execute train_model.py primeiro."
            )
    ...
    if _model is None:
        ...
        if not os.path.exists(model_path):
            ...
            raise FileNotFoundError(
                f"Artefato {model_path} não encontrado. Execute train_model.py primeiro."
            )
...
def predict_churn(raw_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"Inferência concluída. Probabilidade: {probability:.4f} | Churn: {is_churn}"
    )
